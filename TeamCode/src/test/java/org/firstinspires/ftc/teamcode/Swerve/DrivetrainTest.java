...
public class DrivetrainTest {
    ...
    public void setup() {
        ...
        steerEnc = mock(AnalogInput.class);
        imu = mock(IMU.class);
        ...
        when(hw.get(eq(AnalogInput.class), anyString())).thenReturn(steerEnc);
        ...
        when(steerEnc.getVoltage()).thenReturn(0.0);
    ...
    public void testUpdate() {
        dt.update();
        verify(steerEnc, times(3)).getVoltage();
        verify(drv, never()).setPower(anyDouble());
        verify(str, never()).setPower(anyDouble());
    }
